...
export const decompiledMethod = (id: number, instructions: Instr[]): DecompiledMethod => ({
    $: "DecompiledMethod",
    id,
    instructions,
})
...
export const decompiledDict = (methods: DecompiledMethod[]): DecompiledDict => ({
    $: "DecompiledDict",
    methods,
})
...
export const instructionNameForOpcode = (opcode: number): string | undefined => {
    return rangeToName.find(it => opcode >= it.min && opcode < it.max)?.name
}
